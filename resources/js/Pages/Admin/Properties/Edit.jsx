...
import { Head, useForm, Link, router } from '@inertiajs/react'; // Add router import
...
export default function Edit({ property, categories, amenities }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        // handling file upload with PUT requires special cafe in Inertia/Laravel
        // We often use router.post with `_method: 'put'`
        router.post(route('admin.properties.update', property.id), {
            _method: 'put',
            ...data,
            images: newImages, // Only sending new images
        });
    };
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
                                    {property.images && property.images.map((img) => (
                                        <div key={img.id} className="relative aspect-video bg-gray-100 rounded-[6px] overflow-hidden border border-[#e3e4e8] group">
                                            <img src={`/storage/${img.image_path}`} alt="Property" className="object-contain w-full h-full" />
                                            {/* Could add delete button overlay here later */}
                                        </div>
                                    ))}
                                </div>

                                <label className={labelStyle}>Add New Images</label>
